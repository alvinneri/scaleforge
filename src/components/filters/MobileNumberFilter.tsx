...
import { GET_MEMBERS_BY_MOBILE_NUMBER_SEARCH, GET_MEMBERS } from "@/graphql/queries";
...
export const MobileNumberFilter = () => {
  const [search, setSearch] = useState("");
  const { selectedMembers } = useMemberStore();

  const { data: searchData, loading: searchLoading } = useQuery(GET_MEMBERS_BY_MOBILE_NUMBER_SEARCH, {
  ...
  const members = search
    ? searchData?.membersByMobileNumber
        ...
        <DropdownMenuContent className="w-64 p-2" style={{ backgroundColor: '#0A1117', border: '1px solid #2E2E2E', color: '#C2C2C2' }}>
          ...
          <div className="mt-2 max-h-60 overflow-y-auto">
            ...
            {!loading && members && members.length > 0 &&
              members.map((member) => (
                <FilterListItem key={member.id} member={member as Member} displayValue={member.mobileNumber ?? ''} />
              ))}
            {!loading && (!members || members.length === 0) && (
